...
async function main() {
  ...
  updates.recordLaunch();   // rollback bookkeeping; it logs the upgrade itself

  // Must happen before ANY window loads a URL: a stale shell served from cache
  // would otherwise already be running. Keyed off a marker inside the Electron
  // profile rather than off `launch`, so portable builds — whose desktop-state
  // lives beside the executable, away from the profile — are covered too.
  await uiCache.invalidateIfVersionChanged({ log });
