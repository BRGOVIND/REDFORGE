"""Domain answerers for the RedForge Assistant.

Each answerer inspects ONE local data source and returns prose, or ``None`` when
it has nothing to say — never a partial or speculative answer. Everything here is
offline: local database rows and cached metadata only, no model and no network.

Answerers are pure domain logic; they know nothing about HTTP. The ordered chain
that decides which one speaks lives in :mod:`app.assistant.service`.
"""
from __future__ import annotations

import re  # noqa: F401 - several answerers re-import re locally; kept for clarity
from typing import Optional

from sqlalchemy.ext.asyncio import AsyncSession


async def dataset_answer(db: AsyncSession, dataset_id: str, question: str) -> Optional[str]:
...
async def security_evolution_answer(run_id: str, question: str) -> Optional[str]:
...
async def training_answer(db: AsyncSession, run_id: str, question: str) -> Optional[str]:
...
async def recommendation_answer(db: AsyncSession, rec_id: str, question: str) -> Optional[str]:
...
async def benchmark_answer(question: str, project_id: Optional[str]) -> Optional[str]:
...
async def evaluation_answer(question: str, project_id: Optional[str],
